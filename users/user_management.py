...
def add():
	...
	res=requests.post("http://172.17.0.1:8080/api/v1/db/write",json={"insert":d,"column":["name","pass"],"table":"users","indicate":"0"})	
...
def remove(name):
	if(request.method!="DELETE"):
		abort(405,"method not allowed")
	res=requests.post("http://172.17.0.1:8080/api/v1/db/write",json={"table":"users","delete":name,"column":"name","indicate":"1"})
	#print(res.json())
	if(res.json()==0):
...
def list_users():
	...
	src_dest_check=requests.post("http://172.17.0.1:8080/api/v1/db/read",json={"insert":"","column":["name"],"table":"users","where":[]})
...
def clear_db():
	...
	res=requests.post("http://172.17.0.1:8080/api/v1/db/write",json={"indicate":"3"})	
	if(res.json()==0):
