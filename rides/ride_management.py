...
def insert_rider():
	...
	read_res=requests.get("http://172.17.0.1:8080/api/v1/users")
	if(read_res.json()==0):
		abort(400,"no users to fetch")
	if(name not in read_res.json()):
		abort(400,"user doesn't exist")
	
	res=requests.post("http://172.17.0.1:8000/api/v1/db/write",json={"insert":d,"column":["name","timest","source","desti"],"table":"rides","indicate":"0"})	
	if(res.json()==0):
...
def delete_rideId(rideId):
	if(request.method!="DELETE"):
		abort(405,"method not allowed")
	res=requests.post("http://172.17.0.1:8000/api/v1/db/write",json={"table":"rides","delete":rideId,"column":"rideid","indicate":"1"})
	if(res.json()==0):
...
def join_ride(rideId):
	...
	if(name not in read_res.json()):
		abort(400,"user doesn't exist")
	d=[rideId,name]
	rideid_check=requests.post("http://172.17.0.1:8000/api/v1/db/read",json={"insert":d,"column":["rideid","name","source","desti"],"table":"rides","where":["rideid"]})
	if(rideid_check.json().get("response")==0):
		abort(400,"ride id doesn't exists")
	
	res=requests.post("http://172.17.0.1:8000/api/v1/db/write",json={"insert":d,"column":["id","name"],"table":"rideusers","indicate":"0"})
	if(res.json()==0):
		abort(400,"rideId does not  exists")
	elif(res.json()==1):
...
def ride_details(rideId):
	...
	d=[rideId]
	user_list=[]
	rideid_check=requests.post("http://172.17.0.1:8000/api/v1/db/read",json={"insert":d,"column":["rideid","name","source","desti","timest"],"table":"rides","where":["rideid"]})
	if(rideid_check.json().get("response")==0):
		abort(400,"rideId does not  exists")
	elif(rideid_check.json().get("response")==1):
		
		joined_users_check=requests.post("http://172.17.0.1:8000/api/v1/db/read",json={"insert":d,"column":["id","name"],"table":"rideusers","where":["id"]})
...
def upcoming_rides():
	...
	src_dest_check=requests.post("http://172.17.0.1:8000/api/v1/db/read",json={"insert":d,"column":["rideid","name","source","desti","timest"],"table":"rides","where":['source','desti']})			
...
def clear_db():
	...
	res=requests.post("http://172.17.0.1:8000/api/v1/db/write",json={"indicate":"3"})	
	if(res.json()==0):
